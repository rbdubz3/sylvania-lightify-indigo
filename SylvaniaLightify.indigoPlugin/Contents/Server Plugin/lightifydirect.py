...
#
# WIP Python module for Lightify lightify
...
class Lightify:
    ...
    def connect(self):
        ...
        self.__logger.info("...Lightify Connecting...")
    ...
    def send(self, data):
        ...
        try:
            ...
            self.__logger.info('...Caught exception socket.timeout in send')
            return self.connect()
        except (IOError), theError:
            self.__logger.info('...Caught exception IOError, errno="%s"', str(theError.errno))
            if theError.errno == errno.EPIPE or theError.errno == errno.ECONNRESET:
               return self.connect()
            raise theError
        return res
    ...
    def recv(self):
        ...
        try:
            ...
            self.__logger.info('...Caught exception socket.timeout in recv')
            return self.connect()
        except (IOError), theError:
            self.__logger.info('...Caught exception IOError, errno="%s"', str(theError.errno))
            if theError.errno == errno.EPIPE or theError.errno == errno.ECONNRESET:
